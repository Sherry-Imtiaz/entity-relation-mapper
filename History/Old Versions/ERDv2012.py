...
APP_VERSION = "v2.0.2"
APP_VERSION_NAME = "Mermaid Iframe and Return Syntax Patch"
APP_CHANGELOG = [
    {
        "version": "v2.0.2 Patch",
        "title": "Mermaid Iframe and Return Syntax Fix",
        "changes": [
            "Fixed broken newline return syntax that caused an unterminated string literal error.",
            "Replaced deprecated st.components.v1.html usage with st.iframe for Mermaid rendering.",
            "Retained Mermaid zoom, pan, reset, and source download features.",
            "Kept the ERD View Mermaid-only after Graphviz removal."
        ],
    },
    {
        "version": "v2.0.1 Patch",
        "title": "Mermaid Viewer Controls",
        "changes": [
            "Removed Graphviz display from the ERD View workflow.",
            "Added Mermaid-only ERD rendering.",
            "Added zoom in, zoom out, reset, and drag/pan controls to the rendered Mermaid ERD viewer.",
            "Retained Mermaid source code view and Mermaid file download."
        ],
    },
    {
        "version": "v2.0.0 Phase 1",
        "title": "Rendered Mermaid ERD View",
        "changes": [
            "Added direct Mermaid diagram rendering inside the ERD View tab.",
            "Preserved Mermaid source code view and Mermaid file download.",
            "Added ERD display mode selection for rendered Mermaid, Graphviz, or both.",
            "Kept the application in Streamlit and Python while preparing for future visual relationship builder phases."
        ],
    },
    {
...
def render_mermaid_diagram(mermaid_code: str, height: int = 800) -> None:
    """Render Mermaid ERD directly inside Streamlit with zoom, reset, and drag/pan controls."""
    escaped_code = mermaid_code.replace("`", "\`")
    html = f"""
    <div style="font-family: Arial, sans-serif; width:100%;">
        <div style="display:flex; gap:8px; align-items:center; margin-bottom:10px; flex-wrap:wrap;">
            <button id="zoom-in" style="padding:6px 12px; border:1px solid #d1d5db; border-radius:8px; background:#ffffff; cursor:pointer;">Zoom In +</button>
            <button id="zoom-out" style="padding:6px 12px; border:1px solid #d1d5db; border-radius:8px; background:#ffffff; cursor:pointer;">Zoom Out -</button>
            <button id="reset" style="padding:6px 12px; border:1px solid #d1d5db; border-radius:8px; background:#ffffff; cursor:pointer;">Reset</button>
            <span style="font-size:12px; color:#6b7280;">Tip: drag the diagram to pan. Use mouse wheel to zoom.</span>
        </div>
        <div id="mermaid-container" style="width:100%; height:720px; overflow:hidden; border:1px solid #e5e7eb; border-radius:12px; background:#ffffff; cursor:grab;">
            <pre class="mermaid" style="margin:0; padding:16px;">
{escaped_code}
            </pre>
        </div>
    </div>
...
def main() -> None:
    ...
    with tab_erd:
        ...
        if not state.tables:
            ...
            st.markdown("### Rendered Mermaid ERD")
            render_mermaid_diagram(mermaid)

            with st.expander("Mermaid ERD source and download"):
