...
APP_VERSION = "v1.1.0"
APP_VERSION_NAME = "Named Relationship Contexts - Phase 3"
APP_CHANGELOG = [
    {
...
def make_context_id(name: str) -> str:
    ...
    return slug or f"relationship_context_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}"
...
def export_markdown_with_context(state: ErdState, context_id: Optional[str], only_active: bool = True) -> str:
    ...
    return "
".join(header) + "
" + export_markdown(scoped_state, only_active=only_active)
    ...
    with tab_relationships:
        ...
        with st.expander("Relationship validation details"):
            ...
            if validation_df.empty:
                ...
                st.dataframe(validation_df.drop(columns=["id"]), use_container_width=True, hide_index=True)
