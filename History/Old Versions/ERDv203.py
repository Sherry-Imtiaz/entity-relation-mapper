...
APP_VERSION = "v2.0.3"
APP_VERSION_NAME = "Streamlit Flow ERD View Patch"
APP_CHANGELOG = [
    {
...
def get_visual_columns_for_table(table: TableInfo, max_columns: int = 14) -> List[ColumnInfo]:
    """Return visually useful columns, prioritising PK/ID-like fields."""
    columns = sorted(table.columns, key=lambda x: x.ordinal_position)
    priority: List[ColumnInfo] = []
    regular: List[ColumnInfo] = []
    for col in columns:
        cname = col.column_name.lower()
        if col.is_primary_key or cname == "id" or cname.endswith("_id") or cname.endswith("id"):
            priority.append(col)
        else:
            regular.append(col)
    return (priority + regular)[:max_columns]
...
def build_streamlit_flow_state_for_context(state: ErdState, context_id: str) -> Optional[Any]:
    ...
    for idx, table in enumerate(sorted_tables):
        x_pos = (idx % 3) * 380
        y_pos = (idx // 3) * 280
        visual_columns = get_visual_columns_for_table(table)
        column_lines = []
        for col in visual_columns:
            marker = "*" if col.is_primary_key else "•"
            column_lines.append(f"{marker} `{col.column_name}`")
        if len(table.columns) > len(visual_columns):
            column_lines.append(f"… {len(table.columns) - len(visual_columns)} more columns")

        content_lines = [
            f"### {table.full_name}",
            f"**Module:** {table.module or '-'}",
            "---",
        ] + column_lines
        content = line_break.join(content_lines)
...
def build_streamlit_flow_state_from_erd_state(display_state: ErdState) -> Optional[Any]:
    ...
    for idx, table in enumerate(sorted_tables):
        ...
        content_lines = [
            f"### {table.full_name}",
            f"**Rows:** {table.row_count if table.row_count is not None else '-'}",
            f"**Module:** {table.module or '-'}",
            "---",
        ] + column_lines
        ...
        nodes.append(
            StreamlitFlowNode(
                id=table.full_name,
                pos=(x_pos, y_pos),
                data={"content": line_break.join(content_lines)},
                node_type="default",
                source_position="right",
                target_position="left",
                style={
