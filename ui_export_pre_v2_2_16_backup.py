...
from quality_checks import build_export_quality_report, build_scoped_state_from_context, report_to_issue_rows, relationship_tables
...
def _export_markdown(state: ErdState, scope_label: str, only_active: bool = True) -> str:
    lines = [f"# Entity Relation Mapper Export — {scope_label}", "", "## Tables"]
    if not state.tables:
...
def _export_json(state: ErdState, scope_label: str, only_active: bool = True) -> str:
    payload = {
        "scope": scope_label,
        "tables": [
            {"schema_name": table.schema_name, "table_name": table.table_name, "full_name": table.full_name, "columns": [{"column_name": col.column_name, "data_type": col.data_type, "is_primary_key": col.is_primary_key, "nullable": getattr(col, "nullable", ""), "ordinal_position": col.ordinal_position} for col in getattr(table, "columns", []) or []]}
            for table in state.tables.values()
        ],
        "relationships": [
            {"id": rel.id, "context_id": getattr(rel, "context_id", ""), "source_schema": rel.source_schema, "source_table": rel.source_table, "source_column": rel.source_column, "target_schema": rel.target_schema, "target_table": rel.target_table, "target_column": rel.target_column, "relationship_type": rel.relationship_type, "cardinality": rel.cardinality, "join_type": rel.join_type, "condition_sql": rel.condition_sql, "extra_join_sql": getattr(rel, "extra_join_sql", ""), "description": rel.description, "active": rel.active}
...
def _log_export_once(report: dict, export_format: str, scope_label: str) -> None:
    payload = {"format": export_format, "scope": scope_label, "score": report.get("score"), "warning_count": len(report.get("warnings", [])), "relationships_included": report.get("relationships_included"), "tables_included": report.get("tables_included")}
    ...
    try:
        ...
        details = {"format": export_format, "scope": scope_label, "score": report.get("score"), "status": report.get("status"), "warning_count": len(report.get("warnings", [])), "relationships_included": report.get("relationships_included"), "tables_included": report.get("tables_included")}
...
def render_export_tab(state: ErdState) -> None:
    ...
    selected_label = st.selectbox("Relationship Context", list(context_options.keys()), key="export_context_select_v2213", help="Choose the relationship context to export. The export includes relationships in that context.")
    context_id = context_options[selected_label]
    only_active = st.checkbox("Only include active relationships", value=True, key="export_only_active_v2213", help="Exclude inactive relationships from the export and quality report.")
    connected_only = st.checkbox("Only include tables connected to the selected context relationships", value=True, key="export_connected_only_v2213", help="When enabled, tables are inferred from relationship endpoints in the selected context.")
    scoped_state = build_scoped_state_from_context(state, context_id=context_id, active_only=only_active, connected_only=connected_only)
    report = build_export_quality_report(state, context_id=context_id, selected_relationship_ids=list(scoped_state.relationships.keys()), infer_tables_from_relationships=True, active_only=only_active)
    ...
    st.divider()
    export_format = st.selectbox("Export format", ["Markdown", "JSON", "Mermaid"], key="export_format_v2213", help="Markdown is usually best for ChatGPT. JSON is best for structured processing. Mermaid is useful for ERD text.")
    if export_format == "Markdown":
        output = _export_markdown(scoped_state, selected_label, only_active=only_active)
        file_name, mime, language = "entity_relation_mapper_export.md", "text/markdown", "markdown"
    elif export_format == "JSON":
        output = _export_json(scoped_state, selected_label, only_active=only_active)
        file_name, mime, language = "entity_relation_mapper_export.json", "application/json", "json"
    else:
        try:
            output = export_mermaid(scoped_state, only_active=only_active)
        except Exception:
            output = "erDiagram"
        file_name, mime, language = "entity_relation_mapper_export.mmd", "text/plain", "mermaid"
    _log_export_once(report, export_format, selected_label)
    with st.expander("Preview export output", expanded=True):
        st.code(output, language=language)
    st.download_button(f"Download {export_format} export", data=output, file_name=file_name, mime=mime)
