...
def _selected_context_ids_from_ui(state: ErdState) -> tuple[Optional[List[str]], str, bool]:
    ...
    selected_labels = st.multiselect(
        ...
        key="erd_context_multiselect_v2221",
...
def _render_dependency_free_viewer(display_state: ErdState) -> None:
    control_col1, control_col2, control_col3, control_col4 = st.columns(4)

    with control_col1:
        show_relationship_labels = st.checkbox(
            "Show relationship labels",
            value=True,
            key="erd_v2221_show_relationship_labels",
        )

    with control_col2:
        show_condition_labels = st.checkbox(
            "Show condition labels",
            value=True,
            key="erd_v2221_show_condition_labels",
        )
    ...
    viewer_height = st.slider(
        ...
        value=820,
        step=50,
        key="erd_v2221_viewer_height",
    )

    html = build_dependency_free_erd_viewer(
        display_state,
...
def render_erd_tab(state: ErdState) -> None:
    ...
    only_active = st.checkbox(
        ...
        value=True,
        key="erd_only_active_v2221",
    )

    connected_only = st.checkbox(
        "Show only tables connected to selected relationship contexts",
        value=True,
        key="erd_connected_only_v2221",
    )

    display_mode = st.radio(
        "ERD display mode",
        ["Dependency-free interactive viewer", "Static Streamlit Graphviz", "DOT source"],
        index=0,
        horizontal=True,
        key="erd_display_mode_v2221",
        help="The dependency-free viewer supports zoom and pan without Graphviz system installation.",
    )

    display_state = _build_scoped_state(
        state,
    ...
    if display_mode == "Dependency-free interactive viewer":
        _render_dependency_free_viewer(display_state)
    ...
    with st.expander("Why SVG/PNG downloads are disabled by default"):
        st.write(
            "SVG and PNG image export require the Graphviz system executable called `dot`. "
            "That executable is separate from the Python package and is often not available on local "
            "Windows machines or Streamlit Cloud unless installed as a system package. "
            "The dependency-free interactive viewer, DOT download, Mermaid download, and static "
            "Streamlit Graphviz view do not require server-side Graphviz image rendering."
        )
